...
export const LearnMoreButton = ({children, className, ...props}: ComponentPropsWithoutRef<"a"> & LinkProps & MotionProps) => {

  return (
    <Link className={cn("relative group [&_*]:[transition:_all_0.45s_cubic-bezier(0.65,0,0.076,1)] text-white", className)} {...props}>
      <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-10 h-10 bg-blue-500 rounded-full group-hover:w-full`}>
        <span className={cn("absolute left-1 translate-x-0.5 top-0 bottom-0 my-auto w-4 h-0.5 group-hover:bg-white",
